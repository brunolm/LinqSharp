...
export namespace LinqSharp
{
    ...
    export function GetHashCode(e)
    {
        if (e instanceof Number)
            return e.valueOf();

        var s: string = e instanceof Object ? StringifyNonCircular(e) : e.toString();
    ...
    export function StringifyNonCircular(obj)
    {
        var s = s || "";
...
class Linq<T>
{
    ...
    SelectMany<TResult>(selector: (e: T) => T[], resultSelector?: (e: T) => TResult): Linq<TResult>
    {
        ...
        if (!resultSelector)
            return new Linq<TResult>(<any>result);
        else
